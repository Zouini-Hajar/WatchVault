...
const TvShowModal = ({ toggleModal, show, type }) => {
  const [posterURL, setPosterURL] = useState(show ? show[0]?.posterURL : '');
  const [trailerURL, setTrailerURL] = useState(show ? show[0]?.trailerURL : '');
  const [status, setStatus] = useState(show ? show[0]?.watchedStatus : false);
  const [title, setTitle] = useState(show ? show[0]?.title : '');
  const [creators, setCreators] = useState(show ? show[0]?.creators.join('-') : '');
  const [numSeasons, setNumSeasons] = useState(show ? show[0]?.numSeasons : '');
  const [numEpisodes, setNumEpisodes] = useState(show ? show[0]?.numEpisodes : '');
  const [date, setDate] = useState(show ? show[0]?.releaseDate : '');
  const [rating, setRating] = useState(show ? show[0]?.rating : '');
  const [genre, setGenre] = useState(show ? show[0]?.genre.join('-') : '');
  const [cast, setCast] = useState(show ? show[0]?.cast.join('-') : '');
  const [synopsis, setSynopsis] = useState(show ? show[0]?.synopsis : '');
  const [review, setReview] = useState(show ? show[0]?.reviewComments : '');
  ...
  const onUpdate = async () => {
    await axios.put("http://localhost:8888/project/backend/api.php?type=updateShow&id=" + show[0]._id['$oid'], JSON.stringify(values))
  ...
  const onInsert = async () => {
    await axios.post("http://localhost:8888/project/backend/api.php?type=insertShow", JSON.stringify(values))
      .then(response => console.log(response.data))
        ...
        <div className="relative rounded-lg shadow bg-gray-700">
          ...
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-600">
            <h3 className="text-xl font-semibold text-white">
              {type === 'update' ? 'Edit' : 'Add New'} TV Show
            </h3>
          ...
          <div className="p-4 md:p-5 space-y-4">
            <form onSubmit={e => e.preventDefault()}>
              <div className="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6 sm:mb-5">
                  ...
                  <div className='sm:col-span-2'>
                    <div className="mb-5">
                      ...
                      <input
                        ...
                        placeholder="Enter the URL of the TV Show's poster"
                        required
                      />
                    </div>
                    <div className="mb-5">
                      ...
                      <input
                        ...
                        placeholder="Enter the URL of the TV Show's trailer"
                        required
                      />
                    </div>
                    <div className="mb-5">
                      ...
                      />
                      <label htmlFor="status" className="ms-2 text-sm font-medium text-gray-300">
                ...
                <div className="sm:col-span-2">
                  <label htmlFor="title" className="block mb-2 text-sm font-medium text-white">Tv Show's Title</label>
                  <input
                    type="text"
                    name="title"
                    id="title"
                ...
                <div className="sm:col-span-2">
                  <label htmlFor="creators" className="block mb-2 text-sm font-medium text-white">Creators</label>
                  <input
                    type="text"
                    name="creators"
                    id="creators"
                    ...
                    placeholder="Enter the creators seperated by -"
                    required
                  />
                </div>
                <div className="w-full">
                  <label htmlFor="numEpisodes" className="block mb-2 text-sm font-medium text-white">Number of Episodes</label>
                  <input
                    type="number"
                    name="numEpisodes"
                    id="numEpisodes"
                ...
                <div className="w-full">
                  <label htmlFor="numSeasons" className="block mb-2 text-sm font-medium text-white">Number of Seasons</label>
                  <input
                    type="number"
                    name="numSeasons"
                    id="numSeasons"
